...
function initHero(){
	...
	you.kill = function(){
		console.log("hit");
		play = false;
		$("#popup").html("you loose <button id=\"restartlvl\">retry</button>");
		$("#popup").dialog();
	}
...
function killAnim(id_ennemy){
	if(
		...
		totalP += opponents[id_ennemy].value;
		updateScore();
...
function updateScore(){
	document.getElementById("score").textContent(totalP);
}
...
function createScore(){
	loader.load('font/helvetiker_regular.typeface.json', function(font){
		text = new THREE.Mesh(
				new THREE.TextBufferGeometry( 'Hello three.js!', {
					font: font,
					size: 80,
					height: 5,
					curveSegments: 12,
	...
	console.log("create text");
		text.position.set(you.position.x, you.position.y, you.position.z);
		scene.add(text);
}*/
