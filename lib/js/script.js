...
function loadFont(){
	...
	loader.load('three/examples/fonts/gentilis_bold.typeface.json', function(res){
		font = res;
		createText();
		console.log("out");
	});
...
function gestionEnBullet(){
	...
	for (var index = 0; index < enBullets.length; index++) {
		...
		/*for(var loop=0; loop<opponents.length; loop++){
			if(
				enBullets[index].position.x-bullet_x/2 >= opponents[loop].position.x-ennemy_large_x/2 && 
				enBullets[index].position.x+bullet_x/2 <= opponents[loop].position.x+ennemy_large_x/2 &&
				enBullets[index].position.y+bullet_y/2 > opponents[loop].position.y-ennemy_large_y/2 &&
				enBullets[index].position.y+bullet_y/2 < opponents[loop].position.y+ennemy_large_y/2 &&
				enBullets[index].position.z < opponents[loop].position.z+ennemy_large_z/2 &&
				enBullets[index].position.z > opponents[loop].position.z-ennemy_large_z/2 
			){
