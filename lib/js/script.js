...
var frontEnnemy;
var y_min;
var keyboardjs = new THREEx.KeyboardState();
var stats = new Stats();
stats.showPanel(0);
...
function initTerrain(){
	// Generate a terrain
	var t1 = new THREE.ImageUtils.loadTexture('lib/js/three/examples/textures/terrain/grasslight-big.jpg');
	var t2 = new THREE.ImageUtils.loadTexture('lib/js/three/examples/textures/terrain/backgrounddetailed6.jpg');
	var xS = 63, yS = 90;
	...
	terrainScene = THREE.Terrain({
	    ...
	    material: new THREE.Terrain.generateBlendedMaterial([
			{texture: t1},
			{texture: t2, levels: [20, 50,60, 85]}
		]),
	    maxHeight: 50,
	    minHeight: -50,
	    steps: 1,
...
function initHero(){
	...
	you.canShoot = 0;
	you.position.set(0, -10, 20); //placing hero
...
function animate() {
	...
	if (keyboardjs.pressed("left")) { //left
		you.position.x -= 0.1;
	}
	if (keyboardjs.pressed("right")) { //right
		you.position.x += 0.1;
	}
	if (keyboardjs.pressed("up")) { //up
		you.position.y += 0.01;
	}
	if (keyboardjs.pressed("down")) { //down
		you.position.y -= 0.01;
	}
	if (keyboardjs.pressed("pageup")) { //pag.up
		you.position.z += 0.01;
	}
	if (keyboardjs.pressed("pagedown")) { //pag.down
		you.position.z -= 0.01;
	}
	if (keyboardjs['num8']){
...
function aimEnnemy(){
	y_min = -100;
	console.log(y_min);
	for(var u = 0; u < opponents.length; u++){
		if(
			...
			if(opponents[u].position.y > y_min)
			{
				console.log(opponents[u].position.x+" "+opponents[u].position.y);
				y_min = opponents[u].position.y;
				console.log(opponents[u]);
				return opponents[u];
				break;
			}
		}
	}
	return opponents[opponents.length-2];
}
...
function gestionEnBullet(){
	//bullet management
	for (var index = 0; index < enBullets.length; index++) {
