...
function killAnim(id_ennemy){
	if(
		...
		console.log("Ennemy "+id_ennemy+" down");
		opponents[id_ennemy].position.z -=0.3;
	} else {
	...
	for (var index = 0; index < bullets.length; index++) {
		...
		for(var loop=0; loop<opponents.length; loop++){
			if(
				...
				//while(opponents[loop].position.z > plane.position.z+plane.position.z/2){
				//	opponents[loop].position.z -= 0.1;
				//}
